...
![Release Guard components page](Documentation~/assets/components_screen.png)
...
The seeded `Development` profile is intentionally looser than `Release`, but it does not just disable three checks. It leaves some guardrails enabled and disables a broader set of release-only checks. See [Quickstart](Documentation~/quickstart.md) and [Build profiles](Documentation~/build-profiles.md) for the actual seeded defaults.
...
| [Quickstart](Documentation~/quickstart.md) | Install to first blocked build |
| [Configuring](Documentation~/configuring.md) | Minimal settings map |
| [Build profiles](Documentation~/build-profiles.md) | How build-time profile selection actually works |
| [CI integration](Documentation~/guides/ci-integration.md) | Batchmode behavior, manifest, and pipeline implications |
| [Custom components](Documentation~/guides/custom-components.md) | Write your own `ReleaseGuardComponent` |
| [Plugins](Documentation~/api/plugins.md) | Explicit registration and plugin settings |
| [Built-in components overview](Documentation~/reference/components.md) | Every shipped built-in and its role |
