...
{
    public sealed class DebugSymbolSweepTests
    {
        ...
        private ReleaseGuardPostBuildContext Context(
            ReleaseGuardSettings settings, List<ReleaseGuardPostBuildLog> log, string productFileName = "Game.exe")
        ...
        public void FindArtifacts_MatchesKnownUnityArtifacts()
        {
            ...
            var found = DebugSymbolSweep.FindArtifacts(_outputFolder, null);
        ...
        public void FindArtifacts_MatchesExtraPatterns()
        {
            ...
            var found = DebugSymbolSweep.FindArtifacts(
        ...
        public void PostProcess_ReportOnly_WarnsAndDeletesNothing()
        {
            var backup = CreateFolder("Game_BackUpThisFolder_ButDontShipItWithYourGame");
            var s = Settings();
            var log = new List<ReleaseGuardPostBuildLog>();
            try
            {
                var context = Context(s, log);
                context.BeginComponent(new DebugSymbolSweep());

                DebugSymbolSweep.Execute(context);

                Assert.IsTrue(Directory.Exists(backup), "Report-only mode must not delete anything.");
                Assert.AreEqual(1, log.Count(e => e.Level == ReleaseGuardPostBuildLogLevel.Warning),
        ...
        public void PostProcess_Delete_RemovesArtifacts_AndKeepsProduct()
        {
            ...
            try
            {
                ...
                DebugSymbolSweep.Execute(context, new DebugSymbolSweep.Config { delete = true });
                ...
                Assert.AreEqual(2, log.Count(e => e.Level == ReleaseGuardPostBuildLogLevel.Info),
