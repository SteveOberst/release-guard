...
{
    public sealed class ReleaseGuardReportTests
    {
        private static ReleaseIssue Issue(ReleaseIssueSeverity severity) =>
            new ReleaseIssue("auditor", severity, "message");

        private static ReleaseGuardReport Report(params ReleaseIssueSeverity[] severities)
        {
            var issues = new List<ReleaseIssue>();
            foreach (var s in severities)
                issues.Add(Issue(s));
            return new ReleaseGuardReport(issues);
        }
        ...
        {
            var r = new ReleaseGuardReport(null);
            Assert.IsNotNull(r.Issues);
            Assert.IsNotNull(r.DiscoveredAuditors);
            Assert.IsFalse(r.HasIssues);
        }
    }
}