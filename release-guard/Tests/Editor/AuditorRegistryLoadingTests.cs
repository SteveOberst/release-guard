...
namespace ReleaseGuard.Editor.Tests
{
    public sealed class AuditorRegistryLoadingTests
    {
        ...
        public void BuiltInRegistry_ContainsExactlyThirteenAuditors()
        {
            ...
            Assert.AreEqual(13, all.Count, "Built-in count mismatch -- update this test when adding or removing a built-in.");
        ...
        public void TestFixtureAuditors_AreExcludedFromDiscovery()
        {
            ...
            try
            {
                ...
                Assert.IsFalse(ids.Contains("test_discovery_low"),  "[TestAuditorFixture] auditor leaked into discovery.");
                Assert.IsFalse(ids.Contains("test_discovery_high"), "[TestAuditorFixture] auditor leaked into discovery.");
                Assert.IsFalse(ids.Contains("test_provided"),       "[TestAuditorFixture] auditor contributed by a plugin leaked into discovery.");
            }
            finally { Object.DestroyImmediate(s); }
        }
