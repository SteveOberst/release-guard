...
The package id is `org.researchy.release-guard` and it targets Unity `2022.3` and newer. It has no runtime package dependencies.
...
Before each stage the effective configuration is resolved (see [build profiles](guides/build-profiles.md)). When Release Guard is disabled - by the master switch, by a Build Profile override, or by the development-build exemption - every stage skips and logs the reason.

A manual audit from the [audit window](guides/audit-window.md) runs only the auditor stage (step 1) without a build, so it never gates anything and never runs transformers or post-processors.
...
| [API: Plugin settings and custom renderers](api/settings.md) | Declaring plugin settings, custom types, and custom field renderers. |
