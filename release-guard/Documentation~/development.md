...
The runtime assembly (`ReleaseGuard.Runtime`) holds only the public `ReleaseForbiddenAttribute` and `ReleaseIssueSeverity` so gameplay code can reference them without pulling in Editor types. The Editor assembly (`ReleaseGuard.Editor`) holds the pipeline, settings, UI, and built-ins; it is `autoReferenced`, so any Editor assembly in the consuming project can derive from `ReleaseAuditor`, `ReleasePostProcessor`, and `ReleaseTransformer` without adding an explicit asmdef reference.
...
5. Implement `PostProcess(ReleasePostProcessContext context)` and record what you did with `context.Info`, `context.Warning`, and `context.Error`. Do not throw - the executor catches exceptions and turns them into warnings, and a post-processor must never fail the (already successful) build.
...
For project-specific (non-built-in) extensions, derive from the same base types in any Editor assembly and rely on auto-discovery instead of editing these registries - see the Extending section of the documentation.
